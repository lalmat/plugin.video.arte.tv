...
def listVideosNew(url):
    ...
    for item in content["videos"]:
        ...
        try:
            ...
            em = ""
        desc = views+"   |   "+date+"\n"+channels+"\n"+desc
        addLink(cleanTitle(title), em, 'playVideoNew', thumb, desc, duration)
    xbmcplugin.endOfDirectory(pluginhandle)
    if forceViewMode:
...
def playVideoNew(em):
    listitem = xbmcgui.ListItem(path=getStreamUrlNew(em))
    xbmcplugin.setResolvedUrl(pluginhandle, True, listitem)


def getStreamUrlNew(em):
    url = "http://arte.tv/papi/tvguide/videos/stream/player/F/"+em+"_PLUS7-F/ALL/ALL.json"
    '''
